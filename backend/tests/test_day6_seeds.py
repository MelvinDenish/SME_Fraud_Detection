"""Day-6 acceptance tests (PRD §10):
  ...
  - Demo seed assembly finishes in <60 seconds (PRD §13 Definition of Done)
"""
...
async def test_ilfs_fixture_has_five_years_of_financials() -> None:
    """PRD §10 Day 6: 'IL&FS seed data: hand-code FY2014-18 from SFIO report.'"""
...
async def test_ilfs_has_multiple_directors_for_chain_demo() -> None:
    """PRD §14 demo requires the multi-hop director chain on Graph Explorer."""
    bundle = await FixtureSource().fetch_bundle(ILFS_CIN)
...
def test_dhfl_cluster_loads() -> None:
    """PRD §10 Day 6 acceptance: 'DHFL evergreening graph visible.'"""
...
def test_dhfl_has_short_term_charge_cycling_pattern_14() -> None:
    """Pattern 14 (PRD §4.4): 3+ charge create-satisfy cycles, same lender, gap < 45 days.

    DHFL fixture has 3 charges from State Bank of India in 2018-2019, each
    satisfied within ~150 days. The Module 4 query runs on Day 9 — for Day 6
    we just confirm the data is present.
    """
    dhfl = load_dhfl_cluster()
    sbi_dhfl_charges = [
        c for c in dhfl.charges
        if c.cin == "L65910MH1984PLC032526" and c.lender_name == "State Bank of India"
    ]
...
def test_dhfl_has_funded_repayment_for_pattern_13() -> None:
    """Pattern 13 (PRD §4.4): round-trip repayment via shell conduit within 30 days."""
    dhfl = load_dhfl_cluster()
...
def test_dhfl_has_shell_conduits_for_pattern_15() -> None:
    """Pattern 15 (PRD §4.4): shell conduit ≤2 employees, <24 months old, shares director."""
    dhfl = load_dhfl_cluster()
    shells = [c for c in dhfl.companies if c.cin != "L65910MH1984PLC032526"]
    for shell in shells:
    ...
    dhfl_director_cins = {cin for (_, cin) in dhfl.director_cin_links if cin == "L65910MH1984PLC032526"}
    dhfl_dins = {din for (din, cin) in dhfl.director_cin_links if cin == "L65910MH1984PLC032526"}
    shared = False
    ...
    _ = dhfl_director_cins  # silence unused warning — kept for read-clarity


# --- ITC carousel 7-node ring --------------------------------------------
def test_itc_ring_has_seven_nodes() -> None:
    """PRD §10 Day 6 acceptance: 'ITC 7-node ring renders.'"""
    itc = load_itc_carousel()
    assert len(itc.gst_entities) == 7


def test_itc_ring_forms_strongly_connected_component() -> None:
    """SCC = the entire ring. Module 4 Pattern 8 (PRD §4.4) finds this via GDS SCC.
...
def test_itc_ring_has_one_missing_trader() -> None:
    """Pattern 9 (PRD §4.4): tax_paid / itc_generated_for_others < 0.05."""
    itc = load_itc_carousel()
...
# --- Seed timing (PRD §13 Definition of Done) ----------------------------
def test_seed_assembly_under_60_seconds() -> None:
    """PRD §13 Definition of Done: 'Demo seed repopulates Neo4j from scratch in <60s.'
    ...
    ~80 nodes total we're at ~80ms — well inside budget.
    """
    ...
    assert elapsed < PRD_TIME_BUDGET_SECONDS, (
        f"Fixture assembly took {elapsed:.2f}s, exceeds PRD §13 budget of {PRD_TIME_BUDGET_SECONDS}s"
    )
